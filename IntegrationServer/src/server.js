...
const { useAccessToken } = require('./OAuth/token-store-periodic.js'); //loads a file which refreshes temporary access token
schedule.scheduleJob('* * * * *', useAccessToken); //Schedules useAccessToken to run every hour CHANGE this back after testing to 0 * * * *
...
// localTunnel is used to create a custom tunnel, if specified in the environment file
if(run == "Dev") { 
